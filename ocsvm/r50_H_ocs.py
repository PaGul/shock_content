...
individual_params = ConfigSectionMap("Resnet50Hinge2")
...
def train_top_model():
    train_data = np.load(open('/nfs/home/pgulyaev/diploma/ocsvm/resnet50hinge/models/bottleneck_features_train.npy', 'rb'))
    train_labels = np.array(
        # [-1] * 1505 + [1] * 770)
        [-1] * 1490 + [1] * 770)

    validation_data = np.load(open('/nfs/home/pgulyaev/diploma/ocsvm/resnet50hinge/models/bottleneck_features_validation.npy', 'rb'))
...
# потому что сначала скачал шок, а потом нешок (см. метод get_test_inputs)
