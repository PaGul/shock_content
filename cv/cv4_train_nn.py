...
batch_size = 25 
top_model_weights_path = 'bottleneck_fc_modelCV4.h5'
epochs = 100
...
#          [-1] * 2 + [1] * 1)
    ...
    shock_data = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'shock.npy', 'rb'))[1500:2500]
    nonshock_data = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'nonshock.npy', 'rb'))[3090:5150]
    border_data = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'border_data.npy', 'rb'))[60:100]
