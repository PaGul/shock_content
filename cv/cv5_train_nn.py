...
batch_size = 25 
top_model_weights_path = 'bottleneck_fc_modelCV5.h5'
epochs = 100
...
#          [-1] * 2 + [1] * 1)
    ...
    all_shock = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'shock.npy', 'rb'))
    shock_data = all_shock[2000:2500]
    shock_data = np.append(shock_data, all_shock[0:500], axis = 0)
    all_nonshock = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'nonshock.npy', 'rb'))
    nonshock_data = all_nonshock[4120:5150]
    print(nonshock_data.shape)
    nonshock_data = np.append(nonshock_data, all_nonshock[0:1030], axis = 0)
    print(nonshock_data.shape)
    all_border = np.load(open(full_path(un_dir['bottleneck_features_dir'])+'border_data.npy', 'rb'))
    border_data = all_border[80:100]
    border_data = np.append(border_data, all_border[0:20], axis = 0)
