...
im_inputs = get_inputs(whole_shock_dir+class1Name+'/'+class1Name+'*')[500:1000]
im_inputs.extend(get_inputs(whole_nonshock_dir+class2Name+'/'+class2Name+'*')[4120:])
im_inputs.extend(get_inputs(border_data_dir+class2Name+'/*')[40:80])
print(len(im_inputs))
test_activations = get_activations(new_model, im_inputs, RELEVANT_LAYER_NAME)
test_activations.to_csv('activations/testCV3.csv')
